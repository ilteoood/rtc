...
import { ChunkStrategy, getChunk, iterateChunks, split, type SplitOptions } from '..'

describe('split', () => {
  test('should split a single string into correct sizes', () => {
    const input = 'abcdefghij'
    expect(split([input], { chunkSize: 3 })).toStrictEqual([
  ...
  test('should split an array of strings into correct sizes', () => {
    ...
    expect(split(input, { chunkSize: 2 })).toStrictEqual([
  ...
  test('should return the whole string if smaller than chunk size', () => {
    const input = 'abc'
    expect(split([input], { chunkSize: 10 })).toStrictEqual([
      { text: ['abc'], start: 0, end: 3 }
    ])
  })

  test('should handle empty string input', () => {
    expect(split([''], { chunkSize: 5 })).toStrictEqual([
      { text: [''], start: 0, end: 0 }
    ])
  })
  ...
  test('should use default chunk size if not provided', () => {
    ...
    expect((result[0].text[0]).length).toBe(512)
    expect((result[1].text[0]).length).toBe(88)
  ...
  test('should split with overlap (sliding window)', () => {
    const input = 'abcdefghij'
    expect(split([input], { chunkSize: 4, chunkOverlap: 2 })).toStrictEqual([
  ...
  test('should use a custom lengthFunction (count vowels only)', () => {
    ...
    expect(split([input], options)).toStrictEqual([
  ...
  test('should split by paragraph boundaries', () => {
    ...
    const result = split([input], { chunkSize: 100, chunkStrategy: ChunkStrategy.Paragraph })
  ...
  test('should handle empty string with chunkStrategy', () => {
    expect(split([''], { chunkStrategy: ChunkStrategy.Paragraph })).toStrictEqual([
      { text: [''], start: 0, end: 0 }
    ])
  })
  ...
  test('should handle array of empty strings with chunkSize and chunkStrategy', () => {
    expect(
      split(['', ''], { chunkSize: 5, chunkStrategy: ChunkStrategy.Paragraph })
    ).toStrictEqual([
      { text: [''], start: 0, end: 0 },
      { text: [''], start: 0, end: 0 }
    ])
  ...
  test('should cover array input branch for unit-based chunking', () => {
    const input = ['A', 'B', 'C']
    const result = split(input, { chunkSize: 10, chunkStrategy: ChunkStrategy.Paragraph })
    expect(result).toStrictEqual([
      { text: ['A'], start: 0, end: 1 },
  ...
  test('should cover array input mapping for unit-based chunking with multiple non-empty strings', () => {
    ...
    const result = split(input, { chunkSize: 1, chunkStrategy: ChunkStrategy.Paragraph })
    expect(result).toStrictEqual([
      { text: ['A'], start: 0, end: 1 },
      { text: ['B'], start: 1, end: 2 },
      { text: ['C'], start: 2, end: 3 },
...
describe('getChunk', () => {
  test('should return the full string if no start/end provided', () => {
    expect(getChunk(['abcdefgh'])).toStrictEqual(['abcdefgh'])
    expect(getChunk(['abc', 'def', 'gh'])).toStrictEqual(['abc', 'def', 'gh'])
  })

  test('should return substring for start only', () => {
    expect(getChunk(['abcdefgh'], 2)).toStrictEqual(['cdefgh'])
    expect(getChunk(['abc', 'def', 'gh'], 3)).toStrictEqual(['def', 'gh'])
  })

  test('should return substring for start and end', () => {
    expect(getChunk(['abcdefgh'], 2, 5)).toStrictEqual(['cde'])
    expect(getChunk(['abc', 'def', 'gh'], 1, 7)).toStrictEqual(['bc', 'def', 'g'])
  })

  test('should handle end beyond input length', () => {
    expect(getChunk(['abc'], 1, 10)).toStrictEqual(['bc'])
    expect(getChunk(['abc', 'def'], 2, 10)).toStrictEqual(['c', 'def'])
  })

  test('should handle start >= end', () => {
    expect(getChunk(['abc'], 2, 2)).toStrictEqual([''])
    expect(getChunk(['abc', 'def'], 4, 2)).toStrictEqual([])
  })

  test('should handle empty input', () => {
    expect(getChunk([''], 0, 2)).toStrictEqual([''])
    expect(getChunk([], 0, 2)).toStrictEqual([])
  })
})
...
describe('split (coverage edge cases)', () => {
  ...
  test('should handle a single unit larger than chunk size', () => {
    ...
    const result = split([input], { chunkSize: 5, chunkStrategy: ChunkStrategy.Paragraph })
  ...
  test('should cover lengthFunction branch for character-based chunking', () => {
    ...
    expect(result[0]).toStrictEqual({ text: ['ab'], start: 0, end: 2 })
  })
  ...
  test("should cover the 'end === start' branch in character-based chunking", () => {
    ...
    expect(result[0]).toStrictEqual({ text: ['a'], start: 0, end: 1 })
  })
})